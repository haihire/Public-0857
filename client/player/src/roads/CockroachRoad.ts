...
export class CockroachRoad {
  ...
  public getCockroachRoad(
    ...
    for (let j = 0; j < bigRoad[0].length; j++) {
      ...
      for (let i = 0; i < this.row; i++) {
        ...
        if (
          j <= 2 ||
          (i === 0 && j === 3) ||
          (bigRoad[1][3].Win === "" && bigRoad[0][4].Win === "") ||
          bigRoad[i][j].Win === ""
        ) {
          continue;
        }

        if (bigRoad[1][3].Win === "" && bigRoad[0][4].Win === "") {
          continue;
        }
    ...
    this.cockroachRoad = this.getCopy(rows);

    if (inGame) {
      if (Who === "Player") {
        Expects.expectPGun4s = rows[rows.length - 1]
          ? rows[rows.length - 1].Mark
          : "";
      } else if (Who === "Banker") {
        Expects.expectBGun4s = rows[rows.length - 1]
          ? rows[rows.length - 1].Mark
          : "";
      }
    }
  ...
  private getCopy(data: { Mark: string }[]): any[][] {
